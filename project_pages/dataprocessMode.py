...
def map_data(df):
    ...
    df['Stage'] = df['Stage'].apply(lambda x: 2 if str(x) == '-1' else {'early (stage 1 &2)': 0, 'stage 3': 1, 'stage 4': 2}[x.lower()])
    df['Chemotherapy Protocol'] = df['Chemotherapy Protocol'].apply(lambda x: 2 if str(x) == '-1' else {'single agent': 1, 'doublet': 2, 'triplet': 3}[x.lower()])
    ...
    df['Total Platelet count'] = df['Total Platelet count'].apply(lambda x: map_ranges(x, (150000, 450000), default=2))
